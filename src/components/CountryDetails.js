...
const CountryInDetails = () => {
  ...
  useEffect(() => {
    fetch(
      `https://restcountries.com/v2/name/${param.countryname}?fullText=true`
    )
  ...
  useEffect(() => {
    if (
      ...
      Array.isArray(countryResult?.borderCountries)
    ) {
      fetch(
        `https://restcountries.com/v3.1/alpha?codes=${countryResult?.borderCountries.join(
          ","
        )}`
      )
        .then(async (res) => {
  ...
  return (
    <Main isDark={ctx.isDark}>
      <div className="country-details">
        <BackButton isDark={ctx.isDark} onClick={goBack}>
          <i className="fa-solid fa-arrow-left-long"></i> Back
        </BackButton>
        {Object.keys(countryResult).length > 0 ? (
            ...
            <div className="country-details__content__details">
              ...
              <div className="country-details__content__details__information">
                <ul className="country-details__content__details__information__container-1">
                  {Object.entries(countryResult.container1).map(
                    ([key, value], index) => {
                      return (
                        <li key={index}>
                          <b>{`${key}:`}</b>
                ...
                <ul className="country-details__content__details__information__container-2">
                  {Object.entries(countryResult.container2).map(
                    ([key, value], index) => {
                      return (
                        <li key={index}>
                          <b>{`${key}:`}</b>
              ...
              {borderCountries.length > 0 && (
                <div className="border-countries">
                  <span className="border-countries__title">
                    Border Countries:
                  </span>
                    ...
                    {borderCountries.map((value, index) => {
                      return (
                        <Tag
                          isDark={ctx.isDark}
                          onClick={() => {
                            navigate(`/country-api/details/${value}`);
                          }}
                          key={index}
                        >
                          {value}
